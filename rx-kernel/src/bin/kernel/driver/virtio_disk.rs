...
use crate::arch::riscv::qemu::layout::{PGSHIFT, PGSIZE};
...
impl Mutex<Disk> {
    ...
    pub fn rw(&self, buf: &mut Buf, writing: bool) {
        ...
        while guard.info[idx[0]].disk {
            ...
            unsafe {
                CPUManager::myproc()
                    .unwrap()
                    .sleep(buf_raw_data as usize, guard);
            }
            guard = self.lock();
        }
