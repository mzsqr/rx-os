...
use crate::process::cpu::{CPUManager, pop_off, push_off};
...
pub static UART: Mutex<Uart> = Mutex::new(Uart::new(), "uart");
...
impl Uart {
    ...
    pub fn init(&mut self) {
        ...
        write_reg(
            UART_BASE_ADDR + FCR,
            FCR_FIFO_ENABLE as u8 | FCR_FIFO_CLEAR as u8,
        );

        // enable transmit and receive interrupts.
        write_reg(
            UART_BASE_ADDR + IER,
            IER_TX_ENABLE as u8 | IER_RX_ENABLE as u8,
        );
    }
...
pub fn uart_get() -> u8 {
    ...
    let c: u8 = uart_guard.get().expect("Fail to get char");
    drop(uart_guard);
    c
}
...
fn read_reg(addr: usize) -> u8 {
    unsafe { ptr::read(addr as *const u8) }
}
