...
static CONSOLE: Mutex<Console> = Mutex::new(Console::new(), "console");
...
/// or kernel address.
pub(super) fn console_read(is_user: bool, mut dst: usize, size: usize) -> Option<usize> {
    ...
    while left > 0 {
        ...
        while console.read_index == console.write_index {
            let p = unsafe { CPUManager::myproc().expect("Fail to get my process") };
            if p.killed() {
                return None;
            }
...
pub(super) fn console_write(is_user: bool, mut src: usize, size: usize) -> Option<usize> {
    for i in 0..size {
        let mut c = 0u8;
    ...
    match c {
        ...
        _ => {
            ...
            if c != 0 && (console.edit_index - console.read_index).0 < INPUT_BUF {
                ...
                if c == CTRL_LF
                    || c == CTRL_EOT
                    || (console.edit_index - console.read_index).0 == INPUT_BUF
                {
...
use crate::fs::devices::DEVICE_LIST;
...
pub unsafe fn init() {
    ...
    unsafe {
        ...
        DEVICE_LIST.table.as_mut_unchecked()[CONSOLE].write = Some(console_write);
        DEVICE_LIST.table.as_mut_unchecked()[CONSOLE].read = Some(console_read);
    }
}
